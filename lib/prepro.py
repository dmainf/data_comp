...
def normalize_author(df):
    """
    著者名を正規化する（役割表記や括弧内情報の除去、スペースの統一）
    """
    import re
    complex_patterns = [
...
def normalize_title(df):
    ...
    def process_title(title):
        ...
        original = title
        volume = None
        title_without_volume = title
        for pattern in volume_patterns:
            match = re.search(pattern, title)
            if match:
                volume = match.group(1)
                title_without_volume = title[:match.start()].strip()
                break
        base_title = title_without_volume
        for delimiter in ['～', '〜']:
            if delimiter in base_title:
                base_title = base_title.split(delimiter)[0]
        if '（' in base_title:
            if '）' in base_title:
        ...
        if volume:
            volume = volume.translate(str.maketrans('０１２３４５６７８９', '0123456789'))
        if volume:
            result = f"{base_title}_{volume}"
        else:
            result = base_title
